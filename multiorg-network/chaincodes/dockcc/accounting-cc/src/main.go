...
func (s *SmartContract) updateAccountService(APIstub shim.ChaincodeStubInterface, args []string) peer.Response {
	// Check we have a valid number of args
	if len(args) != 2 {
		return shim.Error("Incorrect number of arguments, expecting 2: (1)accountservice host, (2)accountservice port")
	}

	accountServiceHost := args[0]
	accountServicePort := args[1]

	accountServiceURL = accountServiceHost + ":" + accountServicePort
